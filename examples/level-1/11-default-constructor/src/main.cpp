...
public:
    Point(int px = 42, double py = 3.14)
        : x(px)
        , y(py) {
...
public:
    ...
    NoDefaultBecauseOfParams(int v)
        : value(v) {
...
public:
    ...
    NoDefaultBecauseOfMembers(int val, int& r)
        : x(val)
        , ref(r) {
        std::cout << "Parameterized constructor called\n";
    }
...
public:
    ...
    BothConstructors(int i, const std::string& desc)
        : id(i)
        , description(desc) {
...
public:
    ...
    ExplicitlyDefaulted(int val, const std::string& n)
        : x(val)
        , name(n) {}
};
